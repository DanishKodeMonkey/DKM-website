...
const RepoModal: React.FC<RepoModalProps> = ({ repo, onClose }) => {
  ...
  return (
    <div
      className={`modal-overlay transition-opacity duration-500 ease-in-out ${
        isOpen ? "opacity-100" : "opacity-0"
      }`}
      onClick={handleCloseModal}
    >
      <div
        className={`modal-content transition-transform duration-200 ease-in-out transform origin-top ${
          isOpen ? "scale-y-100 opacity-100" : "scale-y-0 opacity-80"
        }`}
